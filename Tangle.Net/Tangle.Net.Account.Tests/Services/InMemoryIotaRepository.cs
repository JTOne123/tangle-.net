namespace Tangle.Net.Account.Tests.Services
{
  ...
  public class InMemoryIotaRepository : IIotaRepository
  {
    ...
    {
      return transactions.Select(t => t.ToTrytes()).ToList();
    }
    ...
    {
      return new TransactionsToApprove { BranchTransaction = Hash.Empty, TrunkTransaction = Hash.Empty, Duration = 0 };
    }

    /// <inheritdoc />
    public async Task<TransactionsToApprove> GetTransactionsToApproveAsync(int depth = 27, Hash reference = null)
    {
      return new TransactionsToApprove { BranchTransaction = Hash.Empty, TrunkTransaction = Hash.Empty, Duration = 0 };
    }
